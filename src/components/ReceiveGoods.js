...
const ReceiveGoods = () => {
  ...
  const normalizeShipmentData = shipment => {
      ...
      products = [
        {
          ...
          batchCode:
            batch.batchNumber || batch.batchCode || shipment.shipmentCode, // ⭐ Use human-readable batch number (BT...)
    ...
    return {
      ...
      trackingCode:
        ...
        `SHIP-${databaseId || "N/A"}`,
      from:
        fromCompany.companyName || fromCompany.pharmacyName || "Nhà phân phối",
      fromType:
        fromCompany.companyType === "MANUFACTURER"
          ? "manufacturer"
          : "distributor",
  ...
  const fetchPendingShipments = async () => {
    try {
      ...
      if (response.success && Array.isArray(response.data)) {
        ...
        const normalized = response.data.map(shipment =>
  ...
  const handleScan = async () => {
    if (!scanInput.trim()) {
      setError(
        "Vui lòng nhập Mã vận đơn (VD: SHIP-17656257365533795) để nhận hàng",
      )
      return
    }
    ...
    try {
      ...
      if (searchTerm.toUpperCase().startsWith("SHIP-")) {
        ...
        console.log(
          "🔢 Numeric input, trying as shipment ID first:",
          shipmentId,
        )
      }
      ...
      if (shipmentId) {
        try {
          ...
          const shipmentResponse =
            await pharmacyService.getShipmentById(shipmentId)
          ...
          if (shipmentResponse.success && shipmentResponse.data) {
            ...
            const toAddr =
              shipment.toAddress || shipment.toCompany?.walletAddress
            const isForMe =
              toAddr?.toLowerCase() === pharmacyAddress?.toLowerCase()
            const isPending =
              shipment.status === "PENDING" || shipment.status === "IN_TRANSIT"

            console.log(
              `✅ Found shipment: toAddr=${toAddr}, isForMe=${isForMe}, status=${shipment.status}`,
            )
            ...
            if (isForMe && isPending) {
              ...
              setError(
                `Lô hàng này đã được nhận (trạng thái: ${shipment.status}).`,
              )
              return
            }
          }
      ...
      try {
        ...
        const shipmentsResponse =
          await pharmacyService.getShipmentsByBatch(searchTerm)
        ...
        if (shipmentsResponse.success && shipmentsResponse.data?.length > 0) {
          ...
          if (myShipment) {
            ...
            setError(
              `Tìm thấy lô thuốc với Số lô: ${searchTerm}, nhưng không có shipment nào đang chờ nhận cho hiệu thuốc của bạn.`,
            )
            return
          }
        }
      ...
      // If all lookups fail
      setError(
        `Không tìm thấy lô hàng với mã: ${searchTerm}. Vui lòng nhập đúng Mã vận đơn (VD: SHIP-17656257365533795).`,
      )
    } catch (err) {
    ...
    try {
      ...
      setError(
        "Lỗi xác nhận nhận hàng: " +
          (err.message ||
            "Không rõ nguyên nhân. Vui lòng kiểm tra kết nối và thử lại."),
      )
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="receive-goods">
      <div className="page-header">
        ...
        <p>
          Quét và xác nhận đã nhận lô hàng, cập nhật quyền giám sát trên
          blockchain
        </p>
      </div>
      ...
      <div className="scanner-section">
        <div className="scanner-card">
          <div className="scanner-header">
            ...
            <p>
              Nhập <strong>Mã vận đơn</strong> (VD: SHIP-17656257365533795) để
              xác nhận nhận hàng.
              <br />
              <span style={{ fontSize: "0.9em", color: "#666" }}>
                💡 Mã vận đơn được NPP/NSX cung cấp khi gửi hàng. Số lô (BT...)
                dùng để truy vết thuốc.
              </span>
            </p>
          </div>
        ...
        <div className="shipment-details">
          <div className="details-card">
            ...
            <div className="details-content">
              ...
              <div
                className="info-section shipment-id-section"
                style={{
                ...
                <h4 style={{ color: "#004085", marginBottom: "12px" }}>
                  🚚 Mã vận đơn (để nhận hàng)
                </h4>
                <div className="shipment-id-display">
                  <div
                    className="shipment-id-value"
                    style={{
                      fontSize: "1.5em",
                      fontWeight: "bold",
                      color: "#004085",
                  ...
                  <p
                    className="shipment-id-note"
                    style={{
                      marginTop: "10px",
                      fontSize: "0.9em",
                      color: "#004085",
                    }}>
                    💡 <strong>Mã vận đơn</strong> dùng để xác nhận nhận hàng.
                    Mã này thay đổi mỗi lần gửi hàng.
                  </p>
                </div>
              </div>
              ...
              <div
                className="info-section batch-number-section"
                style={{
                ...
                <div className="batch-number-display">
                  ...
                  <p
                    ...
                    ⭐ <strong>Số lô này giữ nguyên</strong> từ NSX → NPP → Hiệu
                    thuốc. Dùng để truy vết nguồn gốc thuốc.
                  </p>
                </div>
              </div>
              ...
              <div className="info-section">
                ...
                <div className="info-grid">
                  <div className="info-item">
                    ...
                    <span className="value">
                      ...
                      {shipmentDetails.fromType && (
                        <span
                          className={`source-type ${shipmentDetails.fromType}`}>
                          (
                          {shipmentDetails.fromType === "manufacturer"
                            ? "NSX"
                            : "NPP"}
                          )
                        </span>
                  ...
                  <div className="info-item">
                    ...
                    <span className="value">
                      {formatDate(shipmentDetails.shipmentDate)}
                    </span>
                  </div>
                  <div className="info-item">
                  ...
                  <div className="info-item">
                    ...
                    <span className="value">
                      {shipmentDetails.quantity || 0} sản phẩm
                    </span>
                  </div>
                  <div className="info-item">
                    <span className="label">Tổng giá trị:</span>
                    <span className="value highlight">
                      {formatCurrency(shipmentDetails.totalValue || 0)}
                    </span>
                  </div>
                  <div className="info-item">
                    ...
                    <span className="value">
                      <span
                        className={`status-badge status-${(shipmentDetails.status || "").toLowerCase()}`}>
              ...
              <div className="products-section">
                <h4>
                  Danh sách sản phẩm ({shipmentDetails.products?.length || 0})
                </h4>
              ...
              <div className="blockchain-info">
                ...
                <div className="blockchain-note">
                  <Info size={16} />
                  Khi xác nhận nhận hàng, quyền giám sát sản phẩm sẽ được chuyển
                  từ{" "}
                  <strong>
                    {shipmentDetails.fromType === "manufacturer"
                      ? "Nhà sản xuất"
                  ...
                  sang <strong>Hiệu thuốc</strong> và ghi nhận trên blockchain.
                </div>
              </div>
            </div>
            ...
            <div className="details-actions">
              ...
              <button
                ...
                {loading
                  ? "Đang xác nhận..."
                  : "Xác nhận đã nhận hàng & Cập nhật Blockchain"}
              </button>
            </div>
          </div>
        </div>
              ...
              <div
                ...
                <div className="pending-header-info">
                  ...
                  <div
                    ...
                    <strong style={{ color: "#155724" }}>
                      {shipment.products?.[0]?.batchNumber || "N/A"}
                    </strong>
                  </div>
                </div>
                <div className="pending-content">
                  ...
                  <div className="products-count">
                    {shipment.products?.[0]?.name || "Sản phẩm"} -{" "}
                    {shipment.quantity} hộp
                  </div>
