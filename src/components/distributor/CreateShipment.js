...
function CreateShipment() {
  ...
  const steps = [
    {
      ...
      title: "Chọn lô hàng",
      icon: Package,
      description: "Chọn lô hàng muốn vận chuyển",
    },
    {
      id: 2,
      title: "Chọn điểm đến",
      icon: MapPin,
      description: "Chọn hiệu thuốc nhận hàng",
    },
    {
      id: 3,
      title: "Xác nhận",
      icon: FileText,
      description: "Xác nhận thông tin và gửi đi",
    },
  ]
  ...
  const isStepCompleted = stepId => {
    switch (stepId) {
      ...
      case 3:
        return false // Never completed until final submission
      default:
        return false
    }
  }
  ...
  const handleScanCode = async () => {
    if (!batchCode.trim()) {
      toast.error("Vui lòng nhập mã lô thuốc")
      return
    }
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (
          medicineItems.find(item => item.batchNumber === batchData.batchNumber)
        ) {
          toast.error("Lô thuốc này đã được thêm vào danh sách")
          return
        }
        ...
        toast.success(`Đã thêm thành công: ${medicine.name}`)
      } else {
        toast.error(
          response.message ||
            "Không tìm thấy thông tin lô thuốc với mã: " + batchCode,
        )
      }
    } catch (error) {
      console.error("Error scanning batch code:", error)
      toast.error("Lỗi khi quét mã lô: " + error.message)
    }
  }
  ...
  const handleSubmit = async e => {
    e.preventDefault()
    if (!selectedPharmacy) {
      toast.error("Vui lòng chọn hiệu thuốc nhận hàng")
      return
    }
    if (medicineItems.length === 0) {
      toast.error("Vui lòng thêm ít nhất một lô thuốc")
      return
    }
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.error("Lỗi tạo lô hàng: " + response.message)
      }
    } catch (error) {
      console.error("Error creating shipment:", error)
      toast.error("Lỗi tạo lô hàng: " + error.message)
    }
  }
              ...
              {!showScanner ? (
                ...
                <div className="scanner-form">
                  ...
                  <div className="helper-text">
                    <p>
                      💡 Mã demo có sẵn: LOT2024001, LOT2024002, BT2024001,
                      BT2024002
                    </p>
                  </div>
                </div>
            ...
            {medicineItems.length > 0 && (
              <div className="medicine-list">
                <div className="list-header">
                  <h3>
                    Danh sách thuốc trong lô hàng ({medicineItems.length})
                  </h3>
                    ...
                    filteredPharmacies.map(pharmacy => (
                      <div
                        ...
                        <div className="pharmacy-info">
                          ...
                          {pharmacy.email && (
                            <p className="contact">✉️ {pharmacy.email}</p>
                          )}
                        </div>
                    ...
                    {medicineItems.map(item => (
                      <div
                        ...
                        <div className="medicine-quantity">
                          {parseInt(item.quantity || 0).toLocaleString()} hộp
                        </div>
                        <div className="medicine-batch">
                          Lô: {item.batchNumber}
                        </div>
                      </div>
                    ))}
