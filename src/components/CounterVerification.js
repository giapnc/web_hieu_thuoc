...
const CounterVerification = () => {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      try {
        const response =
          await pharmacyService.verifyBatchAuthenticity(scanInput)
        ...
        if (response.success && response.data.verified) {
          ...
          if (apiTraceabilityHistory && apiTraceabilityHistory.length > 0) {
            blockchainHistorySteps = apiTraceabilityHistory.map(
              (step, index) => ({
            ...
            blockchainHistorySteps = [
              {
                ...
                timestamp:
                  batch.manufactureTimestamp || new Date().toISOString(),
            ...
            if (
              ...
              blockchainHistorySteps.push({
                ...
                timestamp: batch.manufactureTimestamp
                  ? new Date(
                      new Date(batch.manufactureTimestamp).getTime() + 86400000,
                    ).toISOString()
          ...
          verificationResult = {
            ...
            product: {
              ...
              activeIngredient:
                batch.activeIngredient || batch.drugName.split(" ")[0],
              dosage:
                batch.dosage ||
                (batch.drugName.includes("mg")
                  ? batch.drugName.match(/\d+mg/)?.[0] || "N/A"
                  : "N/A"),
              imageUrl: resolveImageUrl(batch.imageUrl),
              description: batch.description,
              manufacturer: batch.manufacturer,
              manufactureDate: batch.manufactureTimestamp
                ? batch.manufactureTimestamp.split("T")[0]
                : "N/A",
            ...
            warnings: [
              {
                ...
                message:
                  response.message ||
                  "Không thể xác thực sản phẩm này trên blockchain",
                severity: "high",
              },
            ],
            recommendations: [
  ...
  const handleSell = async () => {
    ...
    try {
      ...
      const response = await pharmacyService.sellItem(
      ...
      const errorMessage =
        ...
        err.message ||
        "Lỗi khi thực hiện bán hàng"
      setError(errorMessage)

      if (
        errorMessage.includes("đã được bán") ||
  ...
  const submitDamagedReport = async () => {
    if (!damagedReason.trim()) {
      toast.error("Vui lòng nhập lý do hỏng hóc!")
      return
    }
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.error(res.message || "Lỗi khi báo hỏng.")
      }
    } catch (err) {
      toast.error(
        "Không thể báo hỏng sản phẩm: " +
  ...
  const isSold =
    verificationResult?.isSold || verificationResult?.product?.status === "SOLD"
  const isDamaged =
    verificationResult?.isDamaged ||
    verificationResult?.product?.status === "DAMAGED"
  ...
  return (
    <div className="counter-verification">
      <div className="page-header">
        ...
        <p>
          Quét mã QR để xem thông tin nguồn gốc và hình ảnh sản phẩm trước khi
          bán
        </p>
      </div>
      ...
      <div className="scanner-section">
        <div className="scanner-card">
          <div className="scanner-header">
            ...
            <p>
              Quét mã QR hoặc nhập mã lô để xác thực nguồn gốc trước khi bán
            </p>
          </div>
      ...
      {verificationResult && (
        <div className="verification-result">
          ...
          {saleSuccess && (
            <div className="sale-success-banner">
              ...
              <div>
                ...
                <p>
                  Sản phẩm: {saleSuccess.drugName} · Mã: {saleSuccess.itemCode}
                </p>
          ...
          {isDamaged && (
            <div
              ...
              <div>
                ...
                <p>
                  Sản phẩm này đã được báo cáo là có lỗi/nứt vỡ và không thể lưu
                  hành.
                </p>
              </div>
            </div>
          ...
          <div
            className={`result-card ${verificationResult.isAuthentic && !isDamaged ? "authentic" : "suspicious"} ${isSold ? "sold" : ""} ${isDamaged ? "damaged-hero" : ""}`}>
            {/* ── HERO STATUS ── */}
            <div
              className={`result-hero ${verificationResult.isAuthentic && !isDamaged ? "hero-authentic" : "hero-suspicious"}`}>
              <div className="hero-left">
                ...
                <div className="hero-text">
                  ...
                  <p>
                    {isDamaged
                      ? "Đã bị khóa lưu hành trên hệ thống Blockchain"
                      : verificationResult.isAuthentic
                  ...
                  {verificationResult.isAuthentic && (
                    <span
                      className={`sale-status-chip ${isDamaged ? "chip-damaged" : isSold ? "chip-sold" : "chip-available"}`}
                      style={
                        isDamaged
              ...
              {verificationResult.isAuthentic &&
                ...
                !isSold && (
                  <div className="hero-sell-buttons">
                    ...
                    <button
                      className="sell-button sell-with-instructions"
                ...
                <div className="info-grid-new">
                  <div className="info-card-item">
                    <span className="info-card-label">
                      <Tag size={13} /> Mã lô / Batch
                    </span>
                    <span className="info-card-value mono">
                      {verificationResult.product.batchCode}
                    </span>
                  </div>
                  ...
                  <div className="info-card-item">
                    <span className="info-card-label">
                      <Calendar size={13} /> Ngày sản xuất
                    </span>
                    <span className="info-card-value">
                  ...
                  <div className="info-card-item full-width">
                    <span className="info-card-label">
                      <Thermometer size={13} /> Bảo quản
                    </span>
                    <span className="info-card-value">
                      {verificationResult.product.storageConditions ||
            ...
            {/* ── BLOCKCHAIN TIMELINE ── */}
            {verificationResult.blockchainHistory &&
              verificationResult.blockchainHistory.length > 0 && (
                <div className="traceability-section">
                  <div className="section-title-row">
                    <Shield size={18} />
                    <h4>Lịch sử truy xuất nguồn gốc</h4>
                    <span className="step-count-badge">
                      {verificationResult.blockchainHistory.length} bước
                    </span>
                  </div>
                  <div className="blockchain-timeline">
                    {verificationResult.blockchainHistory.map(
                      (event, index) => (
                        <div
                          key={index}
                          ...
                          <div
                            className={`timeline-icon ${getActorColor(event.actorType)}`}>
                          ...
                          <div className="timeline-content">
                            ...
                            {event.shipmentCode && (
                              <div className="shipment-code">
                                📦 Mã vận đơn:{" "}
                                <strong>{event.shipmentCode}</strong>
                              </div>
                            )}
                            {event.txHash && (
                              <div className="tx-hash">
                                ...
                                <span
                                  ...
                                  onClick={() => {
                                    ...
                                    toast.success("Copy TX hash thành công!")
                                  }}
                                  title="Click để copy">
                                  {event.txHash.substring(0, 10)}...
                                  {event.txHash.substring(
                                    event.txHash.length - 8,
                                  )}
                                </span>
                                <a
            ...
            recentVerifications.map(verification => (
              <div
                ...
                <div className="recent-thumb">
                  ...
                  <div
                    className="recent-thumb-fallback"
                    style={{
                      display: verification.imageUrl ? "none" : "flex",
                    }}>
                    <Shield size={18} />
                  </div>
                </div>
          ...
          <div
            ...
            <div
              ...
              <div
                ...
                Hành động này sẽ đánh dấu vĩnh viễn sản phẩm{" "}
                <strong>{verificationResult?.product?.itemCode}</strong> là HỎNG
                trên Blockchain và hệ thống. Bạn không thể hoàn tác!
              </div>
              ...
              <div
                ...
                <small
                  ...
                  Tải lên hình ảnh hộp thuốc bị lỗi để nộp lưu trữ bằng chứng.
                </small>
              </div>
            </div>
